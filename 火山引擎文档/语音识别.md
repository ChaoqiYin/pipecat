...
&lt;Attachment link="[https://portal.volccdn.com/obj/volcfe/cloud-universal-doc/upload_084f2effab285cf82e5196388e0bd354.zip](https://portal.volccdn.com/obj/volcfe/cloud-universal-doc/upload_084f2effab285cf82e5196388e0bd354.zip)" name="sauc_[python.zip](http://python.zip)"&gt;sauc_[python.zip](http://python.zip)&lt;/Attachment&gt;
...
&lt;Attachment link="[https://portal.volccdn.com/obj/volcfe/cloud-universal-doc/upload_25e3ec23373a78508cf36cb446a4141f.zip](https://portal.volccdn.com/obj/volcfe/cloud-universal-doc/upload_25e3ec23373a78508cf36cb446a4141f.zip)" name="sauc_[go.zip](http://go.zip)"&gt;sauc_[go.zip](http://go.zip)&lt;/Attachment&gt;
...
&lt;div data-tips="true" data-tips-type="default"&gt;同时支持&lt;a href="[https://console.volcengine.com/speech/service/10035"&gt;旧版控制台&lt;/a&gt;的鉴权方式，详见：&lt;a](https://console.volcengine.com/speech/service/10035">旧版控制台</a>的鉴权方式，详见：<a) href="[https://docs.volcengine.com/docs/6561/2534847?lang=zh"&gt;旧版控制台鉴权参考示例&lt;/a&gt;&lt;/div&gt;](https://docs.volcengine.com/docs/6561/2534847?lang=zh">旧版控制台鉴权参考示例</a></div>)
...
开启**二遍识别模式，** 默认`false`。开启后，在双向流式实时识别的基础上，对每个分句额外使用非流式模型（nostream）进行二次识别，提升分句的最终准确率。既满足实时上屏的 “快”, 又保证最终结果的 “准”
...
* 仅非流式模型的二次识别结果会携 `"definite": true` 字段，用于标记该分句为最终确定结果

**enable_speaker_info** `bool`

启用说话人分离参数，默认`false`。推荐搭配**豆包流式语音识别模型 2.0** 使用
...
开启后，系统将在分`additions`中返回对应的情绪标签。支持的情绪标签如下：
...
开启后，系统将在分`additions`中返回性别标签（male/female）
...
开启后，系统将在分`additions`中返回说话人的年龄（age），返回值为字符串类型的浮点数
...
指定首字返回加速率，默认值`0`，表示不加速。设置的值越大，首字出字越快。
...
设置音频流起始阶段强制按有声处理的时长，默认值`0`，单位为 ms
...
启用系统内置敏感词库，默认`false`，启用后，命中的系统敏感词会被替换为 `*`
...
启用 POI Function Call，默认`false`，启用后可调用专业的地图领域推荐词服务辅助识别，提高识别准确率
...
启用 Music Function Call，默认`false`，开启后，对于语音识别困难的词语，能调用专业的音乐领域推荐词服务辅助识别
...
* &lt;div data-tips="true" data-tips-type="default"&gt;上下文与热词（含直传热词与热词表热词）最大支持上传100 tokens。当上传的总长度超过 100 tokens 时，系统按传入顺序从前向后截断，仅前 100 tokens 内的内容生效；开启&lt;code&gt;enable_nonstream&lt;/code&gt;参数时，支持情况请参考&lt;a href="[https://docs.volcengine.com/docs/6561/2628951?lang=zh"&gt;单向流式语音识别接口&lt;/a&gt;文档中&lt;code&gt;corpus&lt;/code&gt;对应的描述&lt;/div&gt;](https://docs.volcengine.com/docs/6561/2628951?lang=zh">单向流式语音识别接口</a>文档中<code>corpus</code>对应的描述</div>)
...
* &lt;div data-tips="true" data-tips-type="default"&gt;热词的效果和选词、数量、格式都有关，为达到最佳效果建议根据&lt;a href="[https://docs.volcengine.com/docs/6561/2604976?lang=zh"&gt;热词与上下文最佳实践&lt;/a&gt;配置热词和上下文信息&lt;/div&gt;](https://docs.volcengine.com/docs/6561/2604976?lang=zh">热词与上下文最佳实践</a>配置热词和上下文信息</div>)

**boosting_table_name**  `string`

热词词表名称，可在[控制台&gt;自学习平台]([https://console.volcengine.com/speech/new/hot-word?projectName=default)配置热词后获取](https://console.volcengine.com/speech/new/hot-word?projectName=default)配置热词后获取)

**boosting_table_id**  `string`

热词词表id，可在[控制台&gt;自学习平台]([https://console.volcengine.com/speech/new/hot-word?projectName=default)配置热词后获取](https://console.volcengine.com/speech/new/hot-word?projectName=default)配置热词后获取)
...
替换词词表名称，可在[控制台&gt;自学习平台]([https://console.volcengine.com/speech/new/correct-word?projectName=default)配置替换词后获取。配置后，可将模型识别出的特定词汇替换为目标词汇](https://console.volcengine.com/speech/new/correct-word?projectName=default)配置替换词后获取。配置后，可将模型识别出的特定词汇替换为目标词汇)

**correct_table_id**  `string`

替换词词表id，可在[控制台&gt;自学习平台]([https://console.volcengine.com/speech/new/correct-word?projectName=default)配置替换词后获取。配置后，可将模型识别出的特定词汇替换为目标词汇](https://console.volcengine.com/speech/new/correct-word?projectName=default)配置替换词后获取。配置后，可将模型识别出的特定词汇替换为目标词汇)
...
正则替换词表名称，可在[控制台&gt;自学习平台]([https://console.volcengine.com/speech/new/correct-word?projectName=default)配置正则替换词后获取。相较于替换词的精确匹配替换，正则替换词适合批量格式转换（如日期格式统一、符号标准化）、模糊模式匹配等复杂场景](https://console.volcengine.com/speech/new/correct-word?projectName=default)配置正则替换词后获取。相较于替换词的精确匹配替换，正则替换词适合批量格式转换（如日期格式统一、符号标准化）、模糊模式匹配等复杂场景)

**regex_correct_table_id**  `string`

正则替换词表id，可在[控制台&gt;自学习平台]([https://console.volcengine.com/speech/new/correct-word?projectName=default)配置正则替换词后获取。相较于替换词的精确匹配替换，正则替换词适合批量格式转换（如日期格式统一、符号标准化）、模糊模式匹配等复杂场景](https://console.volcengine.com/speech/new/correct-word?projectName=default)配置正则替换词后获取。相较于替换词的精确匹配替换，正则替换词适合批量格式转换（如日期格式统一、符号标准化）、模糊模式匹配等复杂场景)
...
* &lt;div data-tips="true" data-tips-type="default"&gt;识别效果与热词的选词策略、上下文信息有关，为达到最佳效果建议根据&lt;a href="[https://docs.volcengine.com/docs/6561/2604976?lang=zh"&gt;热词与上下文最佳实践&lt;/a&gt;配置热词和上下文信息&lt;/div&gt;](https://docs.volcengine.com/docs/6561/2604976?lang=zh">热词与上下文最佳实践</a>配置热词和上下文信息</div>)
...
上下文类型，目前仅支`dialog_ctx`

**context_data** `object`

上下文数据列表，用于传入历史对话等语境信息，需同时配`context_type`
...
分词信息列表。请求参`show_utterances`设置`true`且识别成功时返回
